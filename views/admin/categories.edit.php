...
<?php include 'flash.php'; ?>
    ...
    <div class="row">
                        ...
                        <tr>
                            ...
                            <td>
                                <a type="button" class="btn btn-info btn-xs"
                                   title="<?php echo $lang->t('admin|btn_edit'); ?>"
                                   href="<?php _e(ADMIN_MANAGE . "/categories/{$cat->id}/edit"); ?>">
                                    <span class="glyphicon glyphicon-pencil"></span>
                                </a>
                                <a type="button" class="btn btn-danger btn-xs"
                                   title="<?php echo $lang->t('admin|btn_delete'); ?>"
                                   href="<?php _e(ADMIN_MANAGE . "/categories/{$cat->id}/delete"); ?>">
        ...
        <div class="col-md-3">
            <h4><?php if ($category && $category->name) {
            ...
            <form class="form" role="form" method="post" action="<?php _e(ADMIN_MANAGE . '/categories'); ?>">
                <div class="form-group">
                    <input type="text" class="form-control" name="name"
                           placeholder="<?php echo $lang->t('admin|tbl_name'); ?>"
                           value="<?php if ($category && $category->name) {
                               _e($category->name);
                           } ?>" required/>
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" name="description"
                           placeholder="<?php echo $lang->t('admin|tbl_meta_desc'); ?>" maxlength="150"
                           value="<?php if ($category && $category->description) {
                               _e($category->description);
                           } ?>" required/>
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" name="url"
                           placeholder="<?php echo $lang->t('admin|slug_url'); ?>"
                           value="<?php if ($category && $category->url) {
                               _e($category->url);
                           } ?>" required/>
                </div>
                <div class="form-group">
                    <input type="text" class="form-control" name="sort"
                           placeholder="<?php echo $lang->t('admin|tbl_sort'); ?>"
                           value="<?php if ($category && $category->sort) {
                               _e($category->sort);
                           } ?>" required/>
                </div>
                <input type="hidden" name="id" value="<?php if ($category && $category->id) {
                    _e($category->id);
                } ?>">
