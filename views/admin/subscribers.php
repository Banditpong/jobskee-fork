        ...
        <table class="table table-striped">
            ...
            <?php foreach ($users as $user): ?>
                <tr>
                    ...
                    <td><?php if ($user->category_id > 0) {
                            _e($categories[$user->category_id]->name);
                        } ?></td>
                    <td><?php if ($user->city_id > 0) {
                    ...
                    <td>
                        <?php if (!$user->is_confirmed): ?>
                            <a type="button" class="btn btn-success btn-xs"
                               ...
                               href="<?php _e(ADMIN_URL . "subscribers/{$user->id}/approve/{$user->token}"); ?>">
                                <span class="glyphicon glyphicon-ok"></span>
                            </a>
                        <?php else: ?>
                            <a type="button" class="btn btn-warning btn-xs"
                               title="<?php echo $lang->t('admin|btn_deactivate'); ?>"
                               href="<?php _e(ADMIN_URL . "subscribers/{$user->id}/deactivate/{$user->token}"); ?>">
                                <span class="glyphicon glyphicon-minus"></span>
                            </a>
                        <?php endif; ?>
                        <a type="button" class="btn btn-danger btn-xs"
                           title="<?php echo $lang->t('admin|btn_delete'); ?>"
                           href="<?php _e(ADMIN_URL . "subscribers/{$user->id}/delete/{$user->token}"); ?>">
