...
class Parsedown
{
    ...
    private function parse_block_elements(array $lines, $context = '')
    {
        ...
        foreach ($lines as $line) {
            ...
            if ($deindented_line[0] <= '9' and $deindented_line >= '0' and preg_match('/^([ ]*)\d+[.][ ](.*)/', $line,
                    $matches)) {
                $elements [] = $element;
    ...
    private function parse_span_elements($text)
    {
        ...
        if (strpos($text, '](') !== false and preg_match_all('/(!?)(\[((?:[^\[\]]|(?2))*)\])\((.*?)\)/', $text,
                $matches, PREG_SET_ORDER)) # inline
        {
            foreach ($matches as $matches) {
                $url = $matches[4];
        ...
        if ($this->reference_map and strpos($text,
                '[') !== false and preg_match_all('/(!?)\[(.+?)\](?:\n?[ ]?\[(.*?)\])?/ms', $text, $matches,
                PREG_SET_ORDER)) {
            foreach ($matches as $matches) {
